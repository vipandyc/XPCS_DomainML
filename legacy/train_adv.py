...
from produce_data import coarse_grain_g2, normalize_g2
...
class XPCSDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        x = normalize_g2(x, min_val=1.0, max_val=1.2)
        x = x * self.diag_mask
...
class XPCSNet(nn.Module):
    ...
    def __init__(
        ...
        use_shared_feature_mixer: bool = False,
        shared_feature_dim: int = 128,
        shared_feature_mixer_hidden_dim: int = 512,
        predictor_output_activation: str = "linear",
        **legacy_kwargs: Any,
    ):
        super().__init__()
        if "use_prediction_feature_mixer" in legacy_kwargs:
            use_shared_feature_mixer = legacy_kwargs.pop(
                "use_prediction_feature_mixer"
            )
        if "prediction_feature_dim" in legacy_kwargs:
            shared_feature_dim = legacy_kwargs.pop("prediction_feature_dim")
        if "prediction_feature_mixer_hidden_dim" in legacy_kwargs:
            shared_feature_mixer_hidden_dim = legacy_kwargs.pop(
                "prediction_feature_mixer_hidden_dim"
            )
        if legacy_kwargs:
            unexpected_keys = ", ".join(sorted(legacy_kwargs))
            raise TypeError(f"Unexpected XPCSNet kwargs: {unexpected_keys}")
        ...
        self.temp_feature_dim = 32
        self.concatenated_feature_dim = self.xpcs_feature_dim + self.temp_feature_dim
        self.use_shared_feature_mixer = bool(use_shared_feature_mixer)
        self.configured_shared_feature_dim = int(shared_feature_dim)
        self.shared_feature_dim = (
            self.configured_shared_feature_dim
            if self.use_shared_feature_mixer
            else self.concatenated_feature_dim
        )
        self.shared_feature_mixer_hidden_dim = int(shared_feature_mixer_hidden_dim)
        if self.use_shared_feature_mixer and self.configured_shared_feature_dim <= 0:
            raise ValueError("shared_feature_dim must be positive")
        if self.use_shared_feature_mixer and self.shared_feature_mixer_hidden_dim <= 0:
            raise ValueError("shared_feature_mixer_hidden_dim must be positive")
        ...
        self.temp_proj = nn.Sequential(
            nn.Linear(1, 16),
            nn.ReLU(),
            nn.Linear(16, self.temp_feature_dim)
        )

        if self.use_shared_feature_mixer:
            # Build one shared mixed representation that both the predictor and
            # the domain classifier consume.
            self.shared_feature_mixer = nn.Sequential(
                nn.Linear(
                    self.concatenated_feature_dim,
                    self.shared_feature_mixer_hidden_dim,
                ),
                nn.GELU(),
                nn.Linear(
                    self.shared_feature_mixer_hidden_dim,
            ...
            self.shared_feature_mixer = nn.Identity()
    ...
    def get_architecture_config(self) -> Dict[str, Any]:
        ...
        """
        return {
            "use_shared_feature_mixer": self.use_shared_feature_mixer,
            "shared_feature_dim": self.configured_shared_feature_dim,
            "shared_feature_mixer_hidden_dim": self.shared_feature_mixer_hidden_dim,
    ...
    def extract_features(
        ...
        """
        Extract the XPCS and temperature feature branches before task-specific
        heads are applied.
        ...
        Returns:
            ...
            temp_features: Temperature features of shape `(B, 32)`.
        """
        xpcs_features = self.conv_net(x)
        temp_features = self.temp_proj(T)
        return xpcs_features, temp_features
    ...
    def build_shared_features(
        ...
        """
        Fuse the XPCS and temperature branches into one shared representation
        used by both the predictor and the domain classifier.
        ...
        """
        combined_features = torch.cat([xpcs_features, temp_features], dim=1)
        return self.shared_feature_mixer(combined_features)
...
def train(
    ...
    log_dir = log_pardir / f"xpcs_{stamp}"
    ...
    checkpoint_metadata_path = model_path / f"XPCS_best_{stamp}.json"
    ...
    prediction_parameters = (
        ...
        + list(model.temp_proj.parameters())
        + list(model.shared_feature_mixer.parameters())
    ...
    for epoch in range(epochs):
        ...
        if val_selection_loss < best_val_loss - 1e-6:
            ...
            torch.save(model.state_dict(), model_path / f"XPCS_best_{stamp}.pt")
            print(f"Saved best checkpoint (val {best_val_loss:.4f}) -> XPCS_best_{stamp}.pt")
        else:
            bad_epochs += 1
    ...
    load_matching_state_dict(
        best_model,
        torch.load(
            model_path / f"XPCS_best_{stamp}.pt",
            weights_only=True,
            map_location=device,
        ),
    )
...
def build_model_from_checkpoint_metadata(model_path: Path) -> XPCSNet:
    ...
    if metadata_path.exists():
        ...
        architecture_config = canonicalize_architecture_config(
            metadata.get("architecture") or {}
        )
        if architecture_config:
            print(
                "[load] Reconstructing adversarial architecture from "
...
def load_model(
    ...
    to follow the format "XPCS_best_{timestamp}.pt", where {timestamp} is a datetime string
    ...
    if model_path is None:
        ...
        model_files = list(model_dir.glob("XPCS_best_*.pt"))
        if not model_files:
...
if __name__ == "__main__":
    ...
    # best_model = load_model(model_path="models/XPCS_best_20251114-000648.pt")
