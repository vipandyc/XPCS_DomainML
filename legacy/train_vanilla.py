...
class XPCSDataset(Dataset):
    ...
    def __init__(self, paths: Union[Path, List[Path]]):
        ...
        self.mean = 1.1315594972968084
        self.std = 0.011241361147397289
...
class VanillaXPCSNet(nn.Module):
    ...
    def __init__(
        self,
        use_shared_feature_mixer: bool = True,
    ...
    def extract_features(
        ...
        """
        Extract the XPCS and temperature feature branches before the shared
        feature mixer is applied.
        """
...
def train(
    ...
    log_dir = log_pardir / f"vanilla_xpcs_{stamp}"
    ...
    checkpoint_metadata_path = model_path / f"Vanilla_XPCS_best_{stamp}.json"
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss - 1e-6:
            ...
            torch.save(model.state_dict(), model_path / f"Vanilla_XPCS_best_{stamp}.pt")
            print(f"Saved best checkpoint (val {best_val_loss:.4f}) -> Vanilla_XPCS_best_{stamp}.pt")
        else:
            bad_epochs += 1
    ...
    training_metadata.update({
        ...
        "checkpoint_path": str(model_path / f"Vanilla_XPCS_best_{stamp}.pt"),
    })
    save_training_metadata(metadata_path, training_metadata)
    save_training_metadata(checkpoint_metadata_path, training_metadata)
    ...
    best_model.load_state_dict(
        torch.load(
            model_path / f"Vanilla_XPCS_best_{stamp}.pt",
            weights_only=True,
            map_location=device,
        )
    )
...
def build_model_from_checkpoint_metadata(model_path: Path) -> VanillaXPCSNet:
    ...
    if metadata_path.exists():
        ...
        if architecture_config:
            print(
                "[load] Reconstructing vanilla architecture from "
...
def load_model(
    ...
    to follow the format "Vanilla_XPCS_best_{timestamp}.pt", where {timestamp} is a datetime 
    ...
    if model_path is None:
        ...
        model_files = list(model_dir.glob("Vanilla_XPCS_best_*.pt"))
        if not model_files:
...
if __name__ == "__main__":
    ...
    # best_model = load_model(model_path="models/Vanilla_XPCS_best_20251120-232700.pt")
    ...
    combined_df.to_csv("inference_results_vanilla.csv", index=False)
    ...
    print(results_df)
    results_df.to_csv("inference_sim_results_vanilla.csv", index=False)
